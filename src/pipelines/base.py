...
class BigQueryPipelineBase(ABC):
    ...
    def cleanup(self, data: List[BaseModel]) -> None:
        """
        Perform post-load cleanup (e.g., delete from Firestore).
    ...
    def transform(self, raw_data: List[Any]) -> List[dict]:
        ...
        for item in raw_data:
            # If it's already a dict, parse it; otherwise assume it's an object/ORM model
    ...
    def _truncate_staging(self) -> None:
        ...
        logger.info(f"[{self.job_name}] Truncating staging table: {self.staging_table_id}")
    ...
    def _load_to_staging(self, data: List[dict]) -> None:
        ...
        logger.info(f"[{self.job_name}] Loading {len(data)} rows to {self.staging_table_id}...")
    ...
    def _execute_merge(self) -> None:
        ...
        update_list = []
        for col in columns:
            # Skip updating the join keys themselves (though usually harmless if they match)
    ...
    def run(self) -> None:
        ...
        Flow: Extract -> Transform -> Truncate Staging -> Load Staging -> Merge -> Cleanup
