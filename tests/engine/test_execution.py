...
class TestReconciledCloseOptions:
    """Tests for the _get_reconciled_close_options method (Issue #301 Safety)."""

    def test_fallback_to_qty_on_drift(self, execution_engine, mock_trading_client):
        """Verify strict quantity close is used when broker state drifts > 1%."""
        from crypto_signals.domain.schemas import Position, TradeStatus

        # Setup: Broker has significantly more than DB (e.g. manual buy)
        mock_broker_pos = MagicMock()
        mock_broker_pos.qty = "2.0"  # Broker has 2.0
        mock_trading_client.get_open_position.return_value = mock_broker_pos

        position = Position(
            position_id="test-drift-case",
            ds=date(2025, 1, 15),
            account_id="paper",
            symbol="BTC/USD",
            signal_id="test",
            ...
            entry_fill_price=50000.0,
            current_stop_loss=48000.0,
            qty=1.0,  # DB has 1.0 (100% difference)
            side=OrderSide.BUY,
        )

        # Execute
        close_options = execution_engine._get_reconciled_close_options(
            position, percentage=100.0
        )

        # Verify: Should use specific qty, NOT percentage
        assert close_options.percentage is None, "Should not use percentage on drift"
        assert close_options.qty == "1.0", "Should use specific DB quantity on drift"

    def test_use_percentage_when_synced(self, execution_engine, mock_trading_client):
        """Verify percentage close is used when broker state is synced (< 1% diff)."""
        from crypto_signals.domain.schemas import Position, TradeStatus

        # Setup: Broker matches DB perfectly
        mock_broker_pos = MagicMock()
        mock_broker_pos.qty = "1.0"
        mock_trading_client.get_open_position.return_value = mock_broker_pos

        position = Position(
            position_id="test-synced-case",
            ds=date(2025, 1, 15),
            account_id="paper",
            symbol="BTC/USD",
            signal_id="test",
        ...
        # Execute
        close_options = execution_engine._get_reconciled_close_options(
            position, percentage=50.0
        )

        # Verify: Should use percentage
        assert close_options.percentage == "50.0", "Should use percentage when synced"
        assert close_options.qty is None, "Should not use qty when synced"
