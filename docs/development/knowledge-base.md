...
- [2026-01-31] **Documentation Consistency**: "Redundancy Debt" can be avoided by consolidating fragmented documentation (e.g., Catalog + Schema) into a single "Handook". This ensures a single source of truth and reduces the risk of stale fragments.
- [2026-01-31] **Firestore Indices**: Beyond just creating indices, "Index Verification" via the GCP Console's **Explain** feature is vital to ensure that complex queries are actually utilizing the composite indices and not falling back to expensive collection scans.
- [2026-01-31] **Doc Sentinel Persona**: Implementing Jules as a "Doc Sentinel" in PR-gate workflows ensures that documentation is synced with code changes automatically, preventing architectural descriptions from drifting over time.
- [2026-01-31] **Documentation**: "Redundancy Debt" arises when maintaining manual copies of schema definitions (DBML, Catalog, Handbook). Consolidate into a single handbook and prioritize code-first automation (Pydantic introspection) to ensure the "map" stays in sync with the "territory."
